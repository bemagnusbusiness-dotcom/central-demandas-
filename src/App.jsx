...
export default function App() {
  ...
  const load=useCallback(async()=>{
    try{const res=await window.storage.get(STORAGE_KEY,true);setDb(res?JSON.parse(res.value):DEFAULT_DATA);}
    catch{setDb(DEFAULT_DATA);}
    setLoading(false);
  },[]);
  const save=useCallback(async(nd)=>{setSaving(true);try{await window.storage.set(STORAGE_KEY,JSON.stringify(nd),true);}catch(e){console.error(e);}setSaving(false);},[]);
  useEffect(()=>{load();},[load]);
    ...
    if(clientView!==null){
      ...
      return <div className="fu">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
          ...
          {clientTab==="financeiro"&&finTab==="contas"&&isAdmin&&<>
  ...
  const renderModal=()=>{
    ...
    const TitleField=({dis=false})=><div style={{marginBottom:15}}>
      <label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:8}}>Tipo</label>
      <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:10}}>{QT.map(q=>{const a=form._quickType===q.label;return <button key={q.label} type="button" onClick={()=>hQP(q)} disabled={dis} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 13px",borderRadius:20,cursor:dis?"default":"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:12.5,fontWeight:500,transition:"all 0.15s",background:a?"rgba(200,169,110,0.15)":"#1f1f1f",border:`1px solid ${a?"#c8a96e":"#2a2a2a"}`,color:a?"#c8a96e":"#888"}}>{q.icon} {q.label}{a&&" ✓"}</button>;})}</div>
    ...
    const AttachField=()=><div style={{marginBottom:14}}>
    ...
    if(type==="task"||type==="ticket"){const it=type==="ticket";return <>
      ...
      <TitleField dis={!isAdmin} />
      {it&&<div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Cliente *</label><select style={iS} value={form.client??""} onChange={e=>setForm(f=>({...f,client:parseInt(e.target.value)}))} disabled={!isAdmin}>{db.clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Prioridade</label><select style={iS} value={form.priority||"media"} onChange={e=>setForm(f=>({...f,priority:e.target.value}))} disabled={!isAdmin}><option value="baixa">Baixa</option><option value="media">Média</option><option value="alta">Alta</option></select></div>
        <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Status</label><select style={iS} value={form.status||"pendente"} onChange={e=>setForm(f=>({...f,status:e.target.value}))} disabled={!isAdmin}><option value="pendente">Pendente</option><option value="andamento">Em Andamento</option><option value="concluido">Concluído</option></select></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Responsável</label><select style={iS} value={form.assignee??0} onChange={e=>setForm(f=>({...f,assignee:parseInt(e.target.value)}))} disabled={!isAdmin}>{TEAM.map((t,i)=><option key={i} value={i}>{t.name}</option>)}</select></div>
        <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Prazo</label><input type="date" style={iS} value={form.due||""} onChange={e=>setForm(f=>({...f,due:e.target.value}))} disabled={!isAdmin} /></div>
      </div>
      <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#888",marginBottom:6}}>Notas</label><textarea style={{...iS,resize:"vertical",minHeight:70}} value={form.notes||""} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Observações..." disabled={!isAdmin} /></div>
      <AttachField />
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:20}}>{isEdit&&isAdmin&&<Btn variant="danger" onClick={()=>deleteItem(item.id,it?"ticket":"task")}>Excluir</Btn>}<Btn onClick={closeModal}>Cancelar</Btn>{isAdmin&&<Btn variant="primary" onClick={it?saveTicket:saveTask}>Salvar</Btn>}</div>
    </>;}
