...
export default function App() {
              ...
              <div style={{marginBottom:24}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>{contas.map(c=><div key={c.id} style={{background:"#EDE8E2",border:"1px solid #D7CDC1",borderRadius:12,padding:18,position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    ...
                    {c.balance&&<div style={{background:"#FFFFFF",borderRadius:7,padding:"8px 10px"}}><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:"0.07em",color:"#A09080",marginBottom:3}}>Saldo</div><div style={{fontSize:13,color:"#5ab87a",fontWeight:600}}>${parseFloat(c.balance).toLocaleString("en-US",{minimumFractionDigits:2})}</div></div>}
                  </div>
  ...
  const PageSOP=()=>{
    ...
    return <div className="fu">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>{cats.map(cat=>{const items=db.sops.filter(s=>s.category===cat.key);return <Sec key={cat.key} title={cat.label}><div style={{padding:14}}>{items.length===0?<div style={{textAlign:"center",padding:20,color:"#A09080",fontSize:12}}>Nenhum SOP</div>:items.map(s=><div key={s.id} style={{display:"flex",alignItems:"center",gap:12,background:"#EDE8E2",border:"1px solid #D7CDC1",borderRadius:8,padding:"12px 14px",marginBottom:8,cursor:"pointer",transition:"border-color 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(154,122,224,0.4)"} onMouseLeave={e=>e.currentTarget.style.borderColor="#D7CDC1"}><div style={{width:34,height:34,borderRadius:7,background:"rgba(154,122,224,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{s.icon}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{s.title}</div><div style={{fontSize:11,color:"#8B7568",marginTop:2}}>Atualizado: {s.updated}</div></div>{isAdmin&&<button onClick={()=>deleteItem(s.id,"sop")} style={{background:"none",border:"none",color:"#A09080",cursor:"pointer",fontSize:16,padding:"0 4px"}}>×</button>}</div>)}</div></Sec>;})}
      </div>
    </div>;
  };
  ...
  const renderModal=()=>{
    ...
    if(type==="sop")return <>
      <h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:22,color:"#4B2E2B",marginBottom:20}}>{modal.extra?.clientId!==undefined?"Novo SOP do Cliente":"Novo SOP"}</h2>
      <div style={{marginBottom:15}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#7A6558",marginBottom:6}}>Título *</label><input style={iS} value={form.title||""} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="Nome do procedimento..." /></div>
      {modal.extra?.clientId===undefined&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
        ...
        <div><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#7A6558",marginBottom:6}}>Ícone</label><input style={iS} value={form.icon||"📄"} onChange={e=>setForm(f=>({...f,icon:e.target.value}))} placeholder="📄" /></div>
      </div>}
  ...
  return <>
    ...
    <Modal open={!!ccModal} onClose={()=>setCcModal(null)}>
      {ccModal?.type==="conta"&&<>
        ...
        <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#7A6558",marginBottom:6}}>Saldo Atual ($)</label><input style={iS} type="number" value={ccForm.balance||""} onChange={e=>setCcForm(f=>({...f,balance:e.target.value}))} placeholder="0.00" /></div>
      ...
      {ccModal?.type==="cartao"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
          ...
          <div><label style={{display:"block",fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",color:"#7A6558",marginBottom:6}}>4 Últimos Dígitos</label><input style={iS} maxLength={4} value={ccForm.lastFour||""} onChange={e=>setCcForm(f=>({...f,lastFour:e.target.value.replace(/[^0-9]/g,"")}))} placeholder="1234" /></div>
        </div>
